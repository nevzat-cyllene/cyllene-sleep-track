...
function ClockDisplay({ hours, minutes, seconds }: { hours: string; minutes: string; seconds: string }) {
  return (
    <div className="flex items-baseline justify-center tabular-nums tracking-tight">
      <span className="text-[4.25rem] font-extralight leading-none text-white sm:text-[5rem]">
        {hours}
        <span className="text-white/35">:</span>
        {minutes}
      </span>
      <span className="ml-1.5 text-2xl font-light text-white/40 sm:text-3xl">{seconds}</span>
    </div>
  );
}
...
export function SleepModeScreen({
  ...
  return (
    <div className="fixed inset-0 z-[60] flex flex-col bg-[#000000] text-white">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_90%_60%_at_50%_0%,oklch(0.55_0.12_265/14%),transparent_55%)]" />
      ...
      <div className="relative z-10 flex min-h-0 flex-1 flex-col px-5 pb-[max(1.5rem,env(safe-area-inset-bottom))] pt-[max(2.5rem,env(safe-area-inset-top))]">
        ...
        {isFinishing && (
          <div className="mb-5 shrink-0 rounded-[20px] border border-white/[0.08] bg-white/[0.04] px-5 py-4 backdrop-blur-xl">
        ...
        <div className="flex min-h-0 flex-1 flex-col items-center justify-center gap-8">
          <div className="flex items-center gap-2 rounded-full border border-white/[0.06] bg-white/[0.04] px-3.5 py-1.5 backdrop-blur-md">
            {!isFinishing ? (
          ...
          <div className="text-center">
            <p className="mb-3 text-[13px] font-medium text-white/35">Şimdi</p>
            <ClockDisplay hours={wall.hours} minutes={wall.minutes} seconds={wall.seconds} />
          </div>

          <div className="rounded-[22px] border border-white/[0.07] bg-white/[0.03] px-8 py-4 backdrop-blur-xl">
            <p className="mb-2 text-center text-[11px] font-medium text-white/35">
